...
for idx, [rxn_id, smarts] in enumerate(rxn_df.values):
    ...
    for query_idx, col in enumerate(ref_df.columns[2:]):
        ...
        for j in range(0, num_runs):
            config.evaluator_arg.ref_colname = col
            if run_type == "TS":
                res = run_ts(config, hide_progress=True).sort_values("score", ascending=False).head(100)
            elif run_type == "RWS":
                res = run_rws(config, hide_progress=True).sort_values("score", ascending=False).head(100)
            else:
                assert False
